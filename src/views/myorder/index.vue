<template>
  <div class="myorder-container">
    ...
    <!-- 小导航 -->
    <div class="sort-btns">
      <div @click="changegoods($event, all)" class="sort-item all Active">全部</div>
      <div @click="changegoods($event, payment)" class="sort-item payment">待支付</div>
      <div @click="changegoods($event, delivery)" class="sort-item delivery">待发货</div>
      <div @click="changegoods($event, received)" class="sort-item received">待收货</div>
      <div @click="changegoods($event, comment)" class="sort-item ">待评价</div>
    </div>
    ...
    <div class="good-item" v-for="(item, index) in goodsList" :key="index">
      ...
      <div class="item-top">
        ...
        <span class="active">{{ item.state_text }}</span>
      </div>
      <!-- 商品信息 -->
      <div class="item-middle" v-for="itemSon in item.goods" :key="itemSon.goods_id">
        <!-- 商品左侧 -->
        <div class="img-left">
          <!-- 商品图片 -->
          <img :src="itemSon.goods_image" alt="">
        </div>
        <!-- 商品中间说明 -->
        <div class="title-middle">
          <!-- 商品标题 -->
          <div class="item-title text-elipsis-2">{{ itemSon.goods_name }}</div>
        </div>
        <!-- 商品右侧信息 -->
        <div class="info-right">
          <!-- 售出价格 -->
          <span class="item-price price-active">￥{{ itemSon.goods_price }}</span>
          <!-- 售出数量 -->
          <span class="sale-num">x{{ itemSon.total_num }}</span>
        </div>
      </div>
      <!-- 商品下部分 -->
      <div class="item-bottom">
        <p>共{{ totalNum[index] }}件商品，总金额￥{{ item.total_price }}</p>
        <button v-if="item.state_text === '待发货'" @click="cancleOrder(item.order_id)">申请取消</button>
        <p v-else class="cancle">取消申请中...</p>
      </div>
    </div>

    <!-- 空白页 -->
    <div v-if="goodsList.length === 0" class="search-null">
...
export default {
  ...
  methods: {
    // 一次性获取所有订单
    async getOrder () {
      const { all: { list: all }, comment: { list: comment }, received: { list: received }, payment: { list: payment }, delivery: { list: delivery } } = await getOrder()
      this.all = all
      ...
      // 首先获取是否有其他页面传值
      const instruct = this.$route.params.instruct || 'all'
      ...
      this.goodsList = orderMap[instruct].data
      // 给传过来的标签导航高亮
      if (document.querySelector('.Active')) document.querySelector('.Active').classList.remove('Active')
      document.querySelector(`.${instruct}`).classList.add('Active')
    },
    // 获取不同指令的订单
    changegoods (event, obj) {
      this.goodsList = obj.data
      if (document.querySelector('.Active')) document.querySelector('.Active').classList.remove('Active')
  ...
  computed: {
    totalNum () {
      const totallist = this.goodsList.map(item => {
        return (item.goods.reduce((sum, itemson) => (sum + itemson.total_num), 0))
      })
      return totallist
    }
  }
